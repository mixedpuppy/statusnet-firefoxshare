...
"use strict";
dump("loading firefox share from statusnet!\n");
...
function getLogin(domain, activity, credentials) {
...
function logout(domain, activity, credentials) {
  var key = "ff-share-" + domain;
  window.localStorage.removeItem(key);
...
var api = {
  ...
  getProfile: function(activity, credentials) {
    ...
    navigator.apps.oauth.call(oauthConfig, {
      method: "GET",
      action: baseurl+"/api/account/verify_credentials.json",
      parameters: {}
    },function(json) {
  ...
  send: function(activity, credentials) {
    ...
    navigator.apps.oauth.call(oauthConfig, {
      ...
      dump("got statusnet send result "+JSON.stringify(json)+"\n");
      if ('error' in json) {
  ...
  contacts: function(options) {
    ...
    var url = baseurl+"/api/statuses/followers.json";
...
navigator.apps.services.registerHandler('link.send', 'logout', function(activity, credentials) {
  logout(domain, activity, credentials);
});
